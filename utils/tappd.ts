...
function replay_rtmr(history: string[]): string {
  ...
  let mr: Buffer = Buffer.from(INIT_MR, "hex");
  for (const content of history) {
    ...
    if (contentBuffer.length % 64 !== 0) {
      const padding = Buffer.alloc(64 - (contentBuffer.length % 64));
      contentBuffer = Buffer.concat([contentBuffer, padding]);
    }
    const combinedBuffer = Buffer.concat([Buffer.from(mr), contentBuffer]);
    mr = crypto.createHash("sha384").update(combinedBuffer).digest();
  }
