  ...
  return (
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <ol>
          <li>
            This worker agent app is designed to run on Phala Cloud inside a TEE
            and be verified by the worker's smart contract.
          </li>
          <li>
            The app derives a key that is unique to the running instance and TEE
            hardware.
          </li>
          <li>
            The app calls the worker smart contract and registers itself by
            using the remote attestation quote.
          </li>
            ...
            <>
              <a
                ...
                onClick={async () => {
                  ...
                  let res;
                  try {
                    res = await fetch("/api/register").then((r) => r.json());
                  } catch (e) {
                    console.log(e);
                    setMessageHide(
                      "register_worker error: " + JSON.stringify(e, 4)
                    );
                  }
              ...
              <a
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    console.log(e);
                    setMessageHide("get_worker error: " + JSON.stringify(e, 4));
                  }

                  setMessageHide(
                    <>
                      <p>get_worker response:</p>
                      <p className={styles.code}>checksum: {res.checksum}</p>
                      <p className={styles.code}>codehash: {res.codehash}</p>
                    </>
                  );
                }}
              >
              ...
              <a
                ...
                onClick={async () => {
                  ...
                  const res = await fetch("/api/isVerified").then((r) =>
                    r.json()
                  );

                  setMessageHide(
                    <>
                      <p>is_verified_by_codehash response:</p>
                      <p className={styles.code}>
